...
import { countrySchema, leagueSchema } from "src/server/trpc/utils/DTOSchemas";

interface NestedFilterProps {
	items: (typeof countrySchema._type)[];
	h3?: string;
	h2?: string;
	onChange: (countryIds: number[], leagueIds: number[]) => void;
...
const NestedFilter: React.FC<NestedFilterProps> = (props) => {
	...
	function handleSelect(idCountry: number, idChampionship: number | null) {
		const champs = items.find((i) => i.id == idCountry)!.leagues!.map((l) => l.id);

		if (idChampionship == null) {
			// If user selects the block of leagues
			if (
				selectedChampionships.filter((sc) => champs?.includes(sc)).length ==
				champs?.length
			) {
			...
			if (selectedChampionships.includes(idChampionship)) {
				...
				if (
					selectedChampionships.filter((sc) => champs?.includes(sc)).length == 1
				) {
		...
		<div className={styles.itemContainer}>
			<div
				className={`${styles.item} ${
					selectedCountries.includes(id) && styles.active
				}`}
			>
				<div
				...
				{leagues?.map(({ count, id, image, name }) => (
					<div
						className={`${styles.subItem} ${
							selectedChampionships.includes(id) && styles.active
						}`}
