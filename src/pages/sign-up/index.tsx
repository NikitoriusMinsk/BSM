...
const Register: NextPage = () => {
	...
	function handleRegister(e: SyntheticEvent) {
		...
		userMutation
			...
			.catch((r) => {
				setErrors(
					r.data.zodError?.fieldErrors ||
						r.data.serverError?.errorCodes
				)
				if (r.data.httpStatus == 500) {
					let errorTxt: string =
						r.data.serverError?.error ||
						r.data.serverError?.message ||
				...
				<div className={styles.formArea}>
                    ...
                    <span className={styles.separatorOr}>OR</span> */}
						...
						<div className={styles.passwordDescription}>
							...
							<label className={styles.checkTerms}>
								...
								<span className={styles.checkText}>
									...
									{userMutation.error?.data?.zodError
										?.fieldErrors?.terms && (
										<p className={styles.error}>
											{
												userMutation.error?.data
						...
						<div className={styles.formBtns}>
							<SubmitButton disabled={userMutation.isLoading}>
								Sign Up
							</SubmitButton>
						</div>
					</form>
